...
private:
	...
	/*
		Lock tempalte: std::lock()
		It can lock at least tow mutex which is unlimited but cannot
		only one at the same time, which can prevent the dead lock:
		
		If one of lock in std::lock() didn't lock it will wait for there
		until all mutexs were locked then continue to next step.

		Either both mutex are locked or unlock, if only lock one and fail
		to lock another, it will unlock(free) the locked one immediately.
	*/
...
public:
	...
	bool protectGet(int & cmd){
		...
		std::lock_guard<std::mutex> g1(myMutex1, adopt_lock);
